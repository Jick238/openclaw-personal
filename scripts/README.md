...
| Self-hosted full-bundle deployment | `node scripts/hicks-deploy-runtime.mjs --install-root <path> --workspace-root <path> --source-root <checkout> [--dry-run]` | Stages `dist`, package metadata, managed Codex artifacts, versioned Hicks references (including the orchestrator tool profile), doctor preflight, atomic cutover, rollback, and bounded Gateway smoke. Apply the staged `HICKS_ORCHESTRATOR_TOOL_POLICY.json` to the external config atomically before the fresh orchestrator inventory check. Keep config/state outside the install root; `OPENCLAW_UPDATE_WORKSPACE_ROOT` supplies the workspace for `scripts/update-gateway.sh`. |
